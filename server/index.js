...
app.post("/Login", function (req, res) {
  db.get(
    "SELECT * FROM userdetails where Email_Id=? AND password=?;",
    [req.body.Email_Id, req.body.password],
    function (err, rows) {
      if (rows) {
        try {
          if (
            rows.Email_Id == req.body.Email_Id &&
      ...
      try {
        if (
          ...
          if (req.body.Newpassword == req.body.reenternewpassword)
            db.run(
              `UPDATE userdetails SET password = ? WHERE Email_Id = ?`,
    ...
    if (err) {
      ...
      db.all(
        ...
        function (err, uploadata) {
          if (uploadata) {
            db.run(
              `insert into System_Threshold (Scratches,Foreign_Particles,Discoloration,Model)values(?,?,?,?)`,
              ["0%", "0%", "0%", modelname]
            );
            res.send(true);
          } else {
