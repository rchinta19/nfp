  ...
  const nextPage = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        res.data.map((ele) => {
          hdfn.push(
            <Renderhistory key={Math.random().toString()} hditm={ele} />
          );
        });
    ...
    <>
        ...
        <LocalizationProvider dateAdapter={AdapterDateFns}>
          <DatePicker
            ...
            onChange={(newvalue) => {
              ...
              setSkip(0);
              set_From((newvalue) => {
                return `${newvalue.getFullYear()}-${pad2(
                  newvalue.getMonth() + 1
                )}-${pad2(newvalue.getDate())}`;
              });
            }}
            renderInput={(params) => (
              <TextField
      ...
      <div className="pages">
        ...
        <button
          ...
          className="pagebutton"
          style={{
            backgroundColor: historyData.length == 0 ? "grey" : "#0f206c",
          }}
          disabled={historyData.length == 0 ? true : false}
        >
          {" "}
          Next Page{" "}
        </button>
